...
# --- [1. 기본 설정] ---
st.set_page_config(page_title="JJ 쇼츠 마스터 1호점 (자유형)", page_icon="🔥", layout="wide")
...
def create_quiz_image(content_list, d):
    ...
    # content_list에는 (이름, 이미지객체) 튜플이 4개 들어있음
...
with col_L:
    ...
    if mode == "🛠️ 완전 자유 입력(추천)":
        ...
        if 'rand_names' not in st.session_state:
            st.session_state.rand_names = ["윤석열", "이재명", "한동훈", "조국"]
    ...
    with st.container(border=True):
        ...
        with c_q2:
            ...
            if selected_q != VIRAL_QUESTIONS[0]: # 선택이 바뀌면
...
with col_R:
    ...
    if len(final_content) == 4:
        ...
        st.download_button("💾 이미지 다운로드", buf.getvalue(), "shorts_custom.jpg", "image/jpeg", use_container_width=True)
    else:
